...
city_country <- merge(address_city, sql_b_country, by = "country_id")
...
```{r}
#payment amount not in films: but film id is the connector from film to pay
#do somthing similar to last part
sql_c_payment <- gg("SELECT rental_id, 
                            amount 
                     FROM payment")
sql_c_rental <- gg("SELECT rental_id, 
                           inventory_id 
                    FROM rental")
sql_c_inventory <- gg("SELECT inventory_id, 
                              film_id 
                       FROM inventory")
sql_c_film <- gg("SELECT film_id, 
                         title 
                  FROM film")

payment_rental <- merge(sql_c_payment, sql_c_rental, by = "rental_id")

rental_inventory <- merge(payment_rental, sql_c_inventory, by = "inventory_id")

inventory_film <- merge(rental_inventory, sql_c_film, by = "film_id")


max_amount <- max(inventory_film$amount, na.rm = TRUE)
top_rentals <- inventory_film[inventory_film$amount == max_amount, ]
top_rentals[, c("film_id", "title", "amount")]
```
...
```{r}
single_a <- gg("SELECT store_id, 
                       COUNT(customer_id) as total_cust,
                       SUM(active) as active_cust,
                       100 * SUM(active) / COUNT(customer_id) as percent_active
                FROM customer
                GROUP BY store_id")
single_a
```
Since this was a single query I wondered how to add in sums to SQL and used this site as a guide how to use the key words SUM and COUNT <https://www.w3schools.com/sql/sql_count_avg_sum.asp>. 

b.

```{r}
single_b <- gg("SELECT s.staff_id,
                ...
                FROM staff AS s
                      ...
                      INNER JOIN country AS c ON ci.country_id = c.country_id")
...
```{r}
single_c <- gg("SELECT p.rental_id,
                      r.inventory_id,
                      i.film_id,
                      f.title,
                      p.amount
...
single_c
```
Why is scorpion showing up twice? Two different copies of the movie?


Comparisons: have to make them functions in order to compare them. 
...
microbenchmark(sql_r_a, pure_sql_a)

```
From this, it can be seen that, surprisingly, the SQL plus R is faster than the single query for part a. This can be seen looking at the means and seeing SQL plus R has a smaller average run time of 76 nanoseconds compared to 117 for the single query. 
...
microbenchmark(sql_r_b, pure_sql_b)

```
From this we can see that the single query is faster than that on the SQL plus base R for part b. This comes from the fact that the mean run time is shorter than the SQL plus R with a average time of 83 nanoseconds, and this one also has a smaller max run time.
...
microbenchmark(sql_r_c, pure_sql_c)

```
From this, we can see that the single query is faster in this case with a mean of 70 nanoseconds, and the fact it has a smaller max than that of the SQL plus base R version for part c.
